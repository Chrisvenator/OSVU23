...
void client_task_1() {
    ...
    // Task 5: Implementieren Sie die Socket-Erstellung und Initialisierung für den Client.
    if (getaddrinfo(NULL, PORT, &hints, &ai) < 0) err("addrinfo");
...
void client_task_3() {
    ...
    FILE *file_fd = fdopen(socked_fd, "w+");
    fprintf(file_fd, "Nachricht123!");
    fflush(file_fd);

    char buffer[BUFFER_SIZE];
    fgets(buffer, BUFFER_SIZE - 1, file_fd);
...
void server_task_1() {
    ...
    // Task 1: Implementieren Sie die Socket-Erstellung und Initialisierung für den Server.
    if (getaddrinfo(NULL, PORT, &hints, &ai) < 0)err("addr info");
...
void server_task_3() {
    // Task 3: Bringen Sie den Server-Socket dazu, auf eingehende Verbindungen zu warten.
    if (listen(socked_fd, 1) < 0) err("listen");
}
...
void server_task_4() {
    ...
    while (!TERMINATE) {
        ...
        int acc_fd = accept(socked_fd, NULL, NULL);
        if (acc_fd < 0) {
            continue;
            err("accept");
        }
        ...
        size_t rlen = read(acc_fd, buffer, BUFFER_SIZE - 1);
        if (rlen < 0) continue;
        ...
        send(acc_fd, buffer, strlen(buffer), 0);
